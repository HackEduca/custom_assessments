...
def create_script(blocks, blockID):
	...
	while curBlockID is not None:
		...
		counter += 1
		if 'inputs' in curBlockInfo:
			if 'SUBSTACK' in curBlockInfo['inputs']:
				loopBlockID = curBlockInfo['inputs']['SUBSTACK'][1]
				break
		if counter > 100:
			break
	...
	while loopBlockID is not None:
		curBlockInfo = blocks[loopBlockID] #Pull out info about the block
		script[loopBlockID]=curBlockInfo #Add the block itself to the script dictionary
